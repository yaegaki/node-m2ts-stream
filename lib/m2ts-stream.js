...
var M2TsStream = (function () {
	...
	M2TsStream.Packet = M2TsPacket;
	M2TsStream.PSI = require('./m2ts-psi.js');
	M2TsStream.PAT = require('./m2ts-pat.js');
	M2TsStream.PMT = require('./m2ts-pmt.js');
	M2TsStream.PES = require('./m2ts-pes.js');
	...
	M2TsStream.prototype._transform = function (chunk, encoding, callback) {
		if (this._writableState.objectMode) {
			...
			if (this.unitSize < 0) {
				...
				this.unitSize = this._decideUnitSize(this.syncByteOffsets);
			...
			while (this.unitSize > 0 && this.buffer.size() >= this.unitSize) {
				var packetNum = Math.floor(this.buffer.size() / this.unitSize);
				var buf = this.buffer.getContents(this.unitSize * packetNum);
				for (var packetOffset = 0; packetOffset < buf.length; packetOffset += this.unitSize) {
					var rawTsPacket = buf.slice(packetOffset, packetOffset + this.unitSize);
					var pid = -1;
					var tsPacket = null;
	...
	M2TsStream.prototype._decideUnitSize = function (syncByteOffsets, readCount) {
		...
		return this._decideUnitSize(syncByteOffsets, readCount + 1);
	};
	...
	M2TsStream.prototype.unparse = function () {
		var stream = new M2TsStream(this.unitSize);
		stream._setObjectMode(false, this.enableParse);
		return this.pipe(stream);
	};
