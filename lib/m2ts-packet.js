	...
	function M2TsPacket(rawTsPacket) {
		...
		if (this.hasPayload) {
			...
			if (this.payloadUnitStartIndicator) {
				// FIXME: この辺の処理を正しくする
				this.pointerField = this.payload.raw[0];
				if (this.pointerField !== 0) {
					this.pointerFillerBytes = this.payload.raw.slice(1, 1 + this.pointerField);
				} else {
					this.pointerFillerBytes = null;
				}
				
				this.payload.table = this.payload.raw.slice(1 + this.pointerField);
			}
		}
	}
