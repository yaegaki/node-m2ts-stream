var M2TsPacket = (function () {
	...
	function M2TsPacket(rawTsPacket) {
		...
		this.hasTable = (this.adaptationFieldControl & 0x01) > 0;
		...
		if (this.hasTable) {
			this.psiOffset = 0x04 + (this.hasAdaptationField ? 1 + this.adaptationField.size : 0);
			this.pointerField = rawTsPacket[this.psiOffset];
			if (this.pointerField !== 0) {
				this.pointerFillerBytes = rawTsPacket.slice(this.psiOffset, this.psiOffset + this.pointerField);
			} else {
				this.pointerFillerBytes = null;
			}	
			
			this.tableOffset = this.psiOffset + 1 + this.pointerField;
			this.table = {};
