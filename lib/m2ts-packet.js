var M2TsPacket = (function () {
	...
	function M2TsPacket(rawTsPacket) {
		...
		if (this.hasPayload) {
			...
			this.payload = {};
			this.payload.raw = rawTsPacket.slice(this.payloadOffset);
			this.payload.size = this.payload.raw.length;
			
			if (this.payloadUnitStartIndicator) {
				this.pointerField = rawTsPacket[this.payloadOffset];
				if (this.pointerField !== 0) {
					this.pointerFillerBytes = rawTsPacket.slice(this.payloadOffset + 1, this.payloadOffset + 1 + this.pointerField);
				} else {
					this.pointerFillerBytes = null;
				}	
				
				this.tableOffset = this.payloadOffset + 1 + this.pointerField;
				this.table = {};
				this.table.raw = rawTsPacket.slice(this.tableOffset);
				this.table.size = this.table.raw.length;
				this.table.id = this.table.raw[0];

				var section = this.table.section = {};
				section.size = this.table.raw.readUInt16BE(1) & 0x03ff;
				if (section.size > 0) {
					var rawSection = section.raw = this.table.raw.slice(3, 3 + section.size);
					section.idExtension = rawSection.readUInt16BE(0);
					section.number = rawSection[3];
					section.lastSectionNumber = rawSection[4];
					section.data = rawSection.slice(5, rawSection.length - 4);
					section.crc = rawSection.readUInt32BE(rawSection.length - 4);
				}
			}
		}
	}
